 ...
 * @author Masterace
 */
@WebServlet(name = "FuncionServlet", urlPatterns = {"/Funciones"})
...
protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    if (idStr != null && !idStr.isEmpty()) {
        ...
        List<Funcion> funciones = funcionFacade.buscarPorPelicula(idPelicula);
        
        Pelicula peli = peliculaFacade.find(idPelicula);
        
        request.setAttribute("listaFunciones", funciones);
        request.setAttribute("peliculaSeleccionada", peli);
        
        request.getRequestDispatcher("listaFunciones.jsp").forward(request, response);
    } else {
